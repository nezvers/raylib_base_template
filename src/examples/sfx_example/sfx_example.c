...
static void Enter(){
    ...
    button_sfx.sound = LoadSound(RESOURCES_PATH "/sounds/button_sound.wav");
    damage_sfx.sound = LoadSound(RESOURCES_PATH "/sounds/damage_sound.wav");
}

static void Exit(){
    UnloadSound(button_sfx.sound);
    UnloadSound(damage_sfx.sound);
    // CloseAudioDevice(); // should be closed by Settings
}

static void Update(){
    float current_time = GetTime();
    float rand_f = (GetRandomValue(0, 1000000) / 1000000.f) / 1.0f;

    if (IsKeyPressed(KEY_SPACE)) {
        SoundEffectPlayRaylib(&button_sfx, current_time, rand_f);
    }

    if (IsKeyPressed(KEY_ENTER)) {
        SoundEffectPlayRaylib(&damage_sfx, current_time, rand_f);
    }
}
