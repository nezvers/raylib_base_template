...
AppState app_tilemap_example_tileset = {NULL, NULL, NULL, Draw, NULL, "Tileset drawing example"};
...
static void Draw(){
	...
	for (int32_t y = 0; y < ATLAS_SIZE_Y; y += 1) {
		for (int32_t x = 0; x < ATLAS_SIZE_X; x += 1) {
			...
			TileID tile = TilesetGetId(&tileset, (TileID)i);

			TileID atlas_id = TileGetId(&tile_list[tile]);
			Vector2 draw_pos = {
